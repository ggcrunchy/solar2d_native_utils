...
// Custom loader that uses _tcsicoll in lieu of , following https://developertips.blogspot.mx/2007/07/delay-load-dll-and-hrloadallimportsford.html
...
PiddFromDllName(LPCSTR szDll) {
    ...
    if (pinh->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_DELAY_IMPORT].Size != 0) {
        ...
        //    size_t  cchDllCur = __strlen(szDllCur);
            if (/*cchDllCur == __strlen(szDll) && __memcmp*/_stricoll(szDll, szDllCur)/*, cchDllCur)*/ == 0) {
                piddRet = pidd;
                break;
                }
